...
export default function DataTable() {
  ...
  const handleFormSubmit = (event) => {
    ...
    if (Object.keys(errorObj).length === 0) {
      ...
      if (formData.id) {
        axios
          .put(DATA_LIST_URL + "update/" + formData.id, formData, {
            headers: {
  ...
  const handleDelete = (id) => {
    let lt = localStorage.getItem("accessToken");
    axios
      .delete(DATA_LIST_URL + "delete/" + id, {
        headers: {
          Authorization: JSON.parse(lt),
        },
      })
  ...
  const handleSearchClick = async () => {
    try {
      ...
      const response = await axios.get(
        DATA_LIST_URL + "likesearch/" + searchTerm,
        {
          headers: {
            Authorization: JSON.parse(lt),
          },
              ...
              >
                <Grid>
                  ...
                  <Grid align="right">
                    ...
                    ></TextField>
                    <IconButton sx={{ ml: "-50px" }} onClick={handleSearchClick}>
                      <PersonSearchIcon  />
                    </IconButton>
